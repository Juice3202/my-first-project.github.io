function Dashboard() {
  try {
    ...
    const algorithms = [
      { name: 'AES-256', status: 'Активен', security: 'Высокая', icon: 'shield' },
      { name: 'Caesar Cipher', status: 'Активен', security: 'Базовая', icon: 'key' },
      { name: 'Base64', status: 'Активен', security: 'Кодирование', icon: 'code' }
    ];

    return (
      <div className="space-y-8 max-w-7xl mx-auto" data-name="dashboard" data-file="components/Dashboard.js">
              ...
              {getEncryptionHistory().slice(0, 4).map((item, index) => (
                <div key={index} className="flex items-center space-x-3 p-3 bg-[var(--bg-tertiary)] rounded-xl">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                    item.type === 'encrypt' ? 'bg-green-100' : 'bg-blue-100'
                  }`}>
                    <div className={`icon-${item.type === 'encrypt' ? 'lock' : 'unlock'} text-lg ${
                      item.type === 'encrypt' ? 'text-green-600' : 'text-blue-600'
                    }`}></div>
                  </div>
                  <div className="flex-1">
                    <p className="font-medium text-[var(--text-primary)] text-sm">
                      {item.type === 'encrypt' ? 'Шифрование' : 'Расшифровка'}
                    </p>
